...
function runAssetTests(title, resetContracts) {
    tap.test(title + ' specific tests', async (t)=> {
        ...
        // TODO 
        // - test minting fee
        // - test minting from Id
  
        t.test('minting as erc721', async (t) => {
            t.test('minting a NFT (supply = 1) results in erc721 transfer event', async () => {
                const receipt = await tx(contracts.Asset, 'mint', {from: creator, gas}, creator, 0, ipfsHashString, 1, creator);
            ...
            t.test('minting a MCFT (supply > 1) results in no erc721 transfer event', async () => {
                const receipt = await tx(contracts.Asset, 'mint', {from: creator, gas}, creator, 0, ipfsHashString, 100, creator);
                const eventsMatching = await getEventsFromReceipt(contracts.Asset, TransferEvent, receipt);
            ...
            t.test('when fee are enabled minting results the fee collector getting it', async () => {
                ...
                await tx(contracts.Sand, 'transfer', {from: sandOwner, gas}, creator, "1000");
                await tx(contracts.Asset, 'setFeeCollector', {from: mintingFeeCollector, gas}, newFeeCollector, mintingFee);
                await tx(contracts.Asset, 'mint', {from: creator, gas}, creator, 100, ipfsHashString, 1, creator);
            ...
            t.test('when fee are enabled minting results the fee collector contract getting it', async () => {
                ...
                await tx(contracts.Sand, 'transfer', {from: sandOwner, gas}, creator, "1000");
                await tx(contracts.Asset, 'setFeeCollector', {from: mintingFeeCollector, gas}, receiverAddress, mintingFee);
                await tx(contracts.Asset, 'mint', {from: creator, gas}, creator, 100, ipfsHashString, 1, creator);
        ...
        t.test('minting as ERC1155', async (t) => {
            t.test('minting a MCFT (supply > 1) results in erc1155 transfer event', async () => {
                const receipt = await tx(contracts.Asset, 'mint', {from: creator, gas}, creator, 0, ipfsHashString, 4, creator);
