...
contract ERC777MetaTxExtension is ERC20MetaTxExtension {
    bytes32 constant ERC777METATRANSACTION_TYPEHASH = keccak256(
        "ERC777MetaTransaction(address from,address to,uint256 amount,bytes data,uint256 nonce,uint256 gasPrice,uint256 gasLimit,uint256 tokenGasPrice,address relayer)"
    );
    ...
    function sendFrom(
        ...
        require(
            ...
            "only to be used by contract to support meta-tx"
        ); // allow _from to allow gas estimatiom
    ...
    function executeERC777MetaTx(
        ...
        ensureCorrectSigner(
            ...
            _relayer,
            _sig,
            ERC777METATRANSACTION_TYPEHASH,
            signedOnBehalf
        );
        return
            performERC777MetaTx(
    ...
    function performERC777MetaTx(
        ...
        if (_data.length == 0) {
            ...
            (success, returnData) = address(this).call.gas(params[2])(
                abi.encodeWithSignature(
                    "sendFrom(address,address,uint256,bytes)",
                    _from,
                    _to,
                    _amount,
                    _data
